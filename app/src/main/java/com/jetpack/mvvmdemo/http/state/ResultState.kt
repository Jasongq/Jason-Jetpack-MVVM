package com.jetpack.mvvmdemo.http.state

import androidx.lifecycle.MutableLiveData
import com.jetpack.mvvmdemo.data.BaseResponse
import com.jetpack.mvvmdemo.http.exception.AppException
import com.jetpack.mvvmdemo.http.exception.ExceptionHandle

/**
 *  @author : GuoQiang
 *  e-mail : 849199845@qq.com
 *  time   : 2020/07/07  13:48
 *  desc   : 自定义结果集封装类
 *  version: 1.0
 */
...
fun <T> MutableLiveData<ResultState<T>>.paresResult(result: BaseResponse<T>) {
    value = if (result.getCode()==0) ResultState.onAppSuccess(result.getData()) else
        ResultState.onAppError(AppException(result.getCode(), result.getMsg()))
}
